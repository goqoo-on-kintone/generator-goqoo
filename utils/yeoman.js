 ...
 */
const getAllSettingsFromComponentName = (componentName, style, useCssModules, isPure, generatorVersion) => {
  // Use css per default
  if (!style) {
    style = 'css'
  }
  ...
  const cleanedPaths = getCleanedPathName(componentName)
  const componentParts = cleanedPaths.split('/')
  const componentBaseName = _.capitalize(componentParts.pop())
  const componentPartPath = componentParts.join('/')

  // Get the components displayName property
  const componentFullName = _.classify(_.replaceAll(componentName, '/', '_'))

  // Configure Styles
  const stylePaths = configUtils.getChoiceByKey('path', 'style')
  const styleSettings = configUtils.getChoiceByKey('style', style)

  // Configure components
  const componentPath = configUtils.getChoiceByKey('path', 'component')

  // Configure tests
  const testPath = configUtils.getChoiceByKey('path', 'test')
  ...
  switch (generatorVersion) {
    case 4:
      settings = {
        style: {
          webpackPath: `./${componentBaseName.toLowerCase()}${useCssModules ? '.cssmodule' : ''}${
            styleSettings.suffix
          }`,
 ...
 * @return {String}
 */
const getCleanedPathName = (path, suffix) => {
  if (!suffix) {
    suffix = ''
  }
 ...
 * @return {String}
 */
const getComponentStyleName = path => {
  const fileName = path
    .split('/')
    .pop()
    .toLowerCase()
...
const getDestinationPath = (name, type, suffix) => {
  const cleanedPaths = getCleanedPathName(name, suffix)
  const fsParts = cleanedPaths.split('/')
  const actionBaseName = _.capitalize(fsParts.pop())
  const partPath = fsParts.join('/')

  const fsPath = configUtils.getChoiceByKey('path', type).path
 ...
 */
const getDestinationClassName = (name, type, suffix) => {
  const fixedName = getDestinationPath(name, type, suffix)
  return _.capitalize(
    fixedName
      .split('/')
      .pop()
 ...
 */
const getComponentTemplateName = (isStateless, useStyles, useCssModules) => {
